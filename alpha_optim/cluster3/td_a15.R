...
# setwd('/Users/yaolanqiu/Desktop/NYU/Research/FDA/alpha_optimization_2019-09-07/cluster2')
# sample size: 
n = 200
p = 4
...
KL = function(alpha){ # the function 
  ...
  temp_D = solve(D1[2:3, 2:3]) + solve(D2[2:3, 2:3])
  
  if(mean(abs(temp_D)) < 20){
...
for(iters in 1:500){
  ...
  true_KL = c(true_KL, KL_true(alpha))
  inti = alpha + rnorm(p)
...
names = paste('trueD','_a_', angles, '.RData', sep='')
