---
title: "Simulation to estimating alpha 3"
date: 2019-09-15
fontsize: 12pt
output: 
  pdf_document:
...
## Old results (200 subjects each arm) 
...
load('res_d_abs.RData')
colnames(res_d_abs) = c('Angle','Mean KL dif','Mean KL', 'SD KL', 'Mean cosine', 'SD cosine', 'Mean abs cosine', 'SD abs cosine')
kable(res_d_abs)
```
...
* Mean_abs_cosine: the mean value of the absolute cosine_similarity. 

* SD_abs_cosine: The standard deviation value of the absolute cosine_similarity
...
```{r fig.width=8, fig.height=4,include=FALSE,  fig.align = "center"}
##### The histogram of the consine similarity 
img <- readPNG('/Users/yaolanqiu/Desktop/NYU/Research/FDA/alpha_optimization_2019-09-07/cluster2/FDA_alpha2/D_abs.png')
grid.raster(img)
```
...
```{R echo = FALSE}
setwd('/Users/yaolanqiu/Desktop/NYU/Research/FDA/alpha_optimization_2019-09-07/cluster2/FDA_alpha2')
load('res_d2_abs.RData')
colnames(res_d2_abs) = c('Angle','Mean KL dif','Mean KL', 'SD KL', 'Mean cosine', 'SD cosine', 'Mean abs cosine', 'SD abs cosine')
kable(res_d2_abs)
```
...
```{r fig.width=8, fig.height=4, include=FALSE,  fig.align = "center"}
##### The histogram of the consine similarity 
img <- readPNG('/Users/yaolanqiu/Desktop/NYU/Research/FDA/alpha_optimization_2019-09-07/cluster2/FDA_alpha2/trueD_abs.png')
grid.raster(img)
```

## New results (500 each arm)


```{R include = FALSE}
setwd('/Users/yaolanqiu/Desktop/NYU/Research/FDA/alpha_optimization_2019-09-07/cluster4-largeample')
```
...
* 500 subjects in each group, 1000 in total. 
...
```{R echo = FALSE}
load('res_d.RData')
colnames(res_d) = c('Angle','Mean KL dif','Mean KL', 'SD KL', 'Mean cosine', 'SD cosine', 'Mean abs cosine', 'SD abs cosine')
kable(res_d)
```
...
```{R echo = FALSE}
load('res_d2.RData')
res_d2 = res_d2[,c(1,3:9)]
colnames(res_d2) = c('Angle','Mean KL dif','Mean KL', 'SD KL', 'Mean cosine', 'SD cosine', 'Mean abs cosine', 'SD abs cosine')
kable(res_d2)
```
